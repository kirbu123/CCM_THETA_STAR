...
#include "control_mobile_robot_oop/ControlAction.h"
#include "control_mobile_robot_oop/SimulatecollisionAction.h"
...
#include <time.h>
#include <fstream>
typedef actionlib::SimpleActionServer<control_mobile_robot_oop::ControlAction> ControlServer;
using namespace std;



class Controller {
    control_mobile_robot_oop::ControlResult result_msg;
    control_mobile_robot_oop::ControlFeedback feedback_msg;
    control_mobile_robot_oop::SimulatecollisionActionGoal  simulatecollision;//
    ...
    bool StartAction = true;//false;
    bool rotation_collision = true;
    bool position_collision = true;
    bool backward_collision = true;
    bool simulation_node_is_active = false;

    void actionCallback(const control_mobile_robot_oop::ControlGoal::ConstPtr& goal_comand, ControlServer* control_server)
    {
    ...
    void path_theta_back(const nav_msgs::Path& msg) //global planir
    {
        ...
        // ROS_INFO("Goal of theta_star path x , y %f , %f" , msg.poses[size_2-1].pose.position.x , msg.poses[size_2-1].pose.position.y );
        }
    }

    void path_mpc_back(const nav_msgs::Path& msg) //local plan
    {
            ...
            {         
                ...
                path_stored = true;    
                robot_in_tolerance = false;
                k_v=0;
                get_path_errors();
                stop_mode();//доехал или нет
    ...
    //   ROS_INFO("vel 1 %f" , v_cmd);
        //проверка на погрешность, чтобы не ударялся в стену при начале движения
    ...
    void stop_mode()
    {
        ...
        if (dist_stop<=0.21 ) // || robot_in_tolerance // the first enter to this part depends on dist_stop only
        {
            path_stored=false;
            robot_in_tolerance = true;
            if(!flag_orientation ) //&& robot_in_tolerance
            {
                robot_reached=true;
                robot_in_tolerance = false;
            }
    ...
    void define_sub_pub(ros::NodeHandle* n)
    {
        ...
        path_theta_sub =            n->subscribe("/path",10 , &Controller::path_theta_back, this);  //path_back2
        path_mpc_sub   =            n->subscribe("/path",10 , &Controller::path_mpc_back, this);  //path_mpc
    //  direction_sub       = n.subscribe("/detection_passage_server/result",10 , simulate_rotation_back);
        status_collision_sub =      n->subscribe("/collision_rotation",10 , &Controller::collision_rotation_back, this);
        backward_collision_sub =    n->subscribe("/collision_backward",10 , &Controller::collision_backward_back, this);
        position_collision_sub =    n->subscribe("/collision_position",10 , &Controller::collision_position_back, this);

        orient_sub =                n->subscribe(orient_topic,10 , &Controller::flag_last_orientation_back, this);
        odom_sub =                  n->subscribe("/cartographer/tracked_global_odometry", 5, &Controller::odom_back, this);
        cmd_pub =                   n->advertise<geometry_msgs::Twist>(cmd_topic, 1, this);
        grid_map_sub =              n->subscribe("/occupancy_grid_map/grid_map", 10, &Controller::grid_map_back, this);  
        grid_local_map_sub =        n->subscribe("/occupancy_grid_local_map/grid_map", 10, &Controller::grid_local_map_back, this);
        //detection_passage_pub =     n->advertise<control_mobile_robot_oop::SimulatecollisionActionGoal>("/detection_passage_server/goal", 1, this);
        reached_status_pub =        n->advertise<std_msgs::String> ("reached_status", 10, this);
    }
...
public:
    void run(int argc, char **argv) 
    {
        ...
        //ControlServer server(n, "control_motion", boost::bind(&Controller::actionCallback, _1, &server, this), false);
        //ControlServer server(n, "control_motion", boost::bind(&Controller::actionCallback, this, _1, &server), false);
        //server.start();
        ...
        while (ros::ok()) 
        {
            ...
            try
            {    
                transformStamped = tfBuffer.lookupTransform(world_frame, base_frame,
                                ros::Time(0));
            }
...
*/
            ...
            */
            ...
            // сим нод из акстив означает что получили ответ от ноды, которая может сделать движение, если не получили, тогда не можем сделать движение
