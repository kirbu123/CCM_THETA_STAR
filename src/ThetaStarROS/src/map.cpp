...
Node Map::GetClosestNode(const Point &point) const
{
    ...
    quaternion.setValue(-originOrientation.x, -originOrientation.y, -originOrientation.z, originOrientation.w);

    tf2::Vector3 vector(point.X() - originPosition.X(), point.Y() - originPosition.Y(), 0.0);

    vector = tf2::quatRotate(quaternion, vector);

    Node res;

    res.i = static_cast<int>((vector.y() / cellSize));
    res.j = static_cast<int>((vector.x() / cellSize));
...
Point Map::GetPoint(const Node &node) const
{
    ...
    tf2::Vector3 vector((node.j + 0.5f) * cellSize, (node.i + 0.5f) * cellSize, 0.0);

    vector = tf2::quatRotate(quaternion, vector);

    auto pos_x = originPosition.X() + vector.x();
    auto pos_y = originPosition.Y() + vector.y();
