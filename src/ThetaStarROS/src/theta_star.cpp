...
bool ThetaStar::SearchPath(const Node &start, const Node &goal)
{
    ...
    //std::cout << start.i << " " << start.j << " " << map->CellIsObstacle(start.i, start.j) << "\n";
    //std::cout << goal.i << " " << goal.j << " " << map->CellIsObstacle(goal.i, goal.j) << "\n";
    Node curNode;
    curNode.i = start.i;
    curNode.j = start.j;
    curNode.g = 0;
...
bool ThetaStar::CreateGlobalPath()
{
    if(!glPathCreated)
    {
        std::cout << "start: " << glStart.X() << ' ' << glStart.Y() << '\n';
        std::cout << "goal: " << glGoal.X() << ' ' << glGoal.Y() << '\n';
        Node start = map->GetClosestNode(glStart);
        Node goal  = map->GetClosestNode(glGoal);
        //my code start
        ROS_INFO("Debug commenraries:");
        std::cout << "start XY: " << start.i << ' ' << start.j << '\n';
        std::cout << "goal XY: " << goal.i << ' ' << goal.j << '\n';
        //my code end
