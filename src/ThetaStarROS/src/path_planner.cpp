...
{
    ROS_INFO("Received task at time %f", ros::Time::now().toSec());
    std::list<Point> path;
    ...
    ROS_INFO("Path search finished at time %f", ros::Time::now().toSec());

    // publish path as nav_msgs::Path
    geometry_msgs::PoseStamped pose;
    ROS_INFO("Found path with size: %d", path.size());
    ...
    std::cout << "ARRAY SIZE: " << path_msg.poses.size() << '\n';
