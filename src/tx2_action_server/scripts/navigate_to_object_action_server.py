...
class MoveRobot:
    ...
    def wait_for_path(self):
        while self.path is None or abs(self.path[-1][0] - self.object_x) > 0.25 or abs(self.path[-1][1] - self.object_y) > 0.25:
    ...
    def execute(self, goal):
        ...
        if goal.location in self.locations:
            ...
            if self.found:
                ...
                path = self.wait_for_path()
                rospy.loginfo('Done!')
                object_reach_distance = 1.0
